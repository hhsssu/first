...
CREATE TABLE category (
    cate_code NUMBER(2) CONSTRAINT cate_code_pk PRIMARY KEY,
...
DELETE FROM product WHERE product_code = 21;
...
ALTER TABLE product ADD product_img1 VARCHAR2(500);
ALTER TABLE product ADD product_img2 VARCHAR2(500);
ALTER TABLE product ADD product_img3 VARCHAR2(500);
ALTER TABLE product ADD product_img4 VARCHAR2(500);
ALTER TABLE product ADD product_img5 VARCHAR2(500);
...
SELECT case when COUNT(product_code)>=1 then 'true' else 'false' end as isExisted
     FROM cart
        WHERE
    customer_id = 'api@naver.com'
    AND 
...
SELECT IF(COUNT(product_code) = 1, 1, 0)
     FROM cart
        WHERE
    customer_id = 'api@naver.com'
    AND 
...
-- 컬럼 추가
ALTER TABLE cart ADD product_code NUMBER(10);
...
DELETE FROM cart WHERE cart_code = 1;
...
-- 컬럼 추가
ALTER TABLE product_order ADD product_code NUMBER(10);
